    ...
    public class InMemoryDirectoryHandler : IDirectoryHandler
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var absolutePath = new AbsolutePath(
                absolutePathString,
                true,
                _commonService.FileSystemProvider,
            ...
            _commonService.FileSystemProvider.DeletionPermittedRegister(
                new SimplePath(
                    absolutePathString,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var childDirectories = (await GetDirectoriesAsync(sourceAbsolutePathString, cancellationToken).ConfigureAwait(false))
                .Select(x => new AbsolutePath(x, true, _commonService.FileSystemProvider, tokenBuilder: new StringBuilder(), formattedBuilder: new StringBuilder(), AbsolutePathNameKind.NameWithExtension))
                .ToArray();

            var childFiles = (await GetFilesAsync(sourceAbsolutePathString, cancellationToken).ConfigureAwait(false))
                .Select(x => new AbsolutePath(x, false, _commonService.FileSystemProvider, tokenBuilder: new StringBuilder(), formattedBuilder: new StringBuilder(), AbsolutePathNameKind.NameWithExtension))
                .ToArray();
            ...
            {
                var innerDestinationPath = _commonService.FileSystemProvider.JoinPaths(
                    destinationAbsolutePathString,
                    sourceAbsolutePath.Name);

                var destinationChild = _commonService.FileSystemProvider.JoinPaths(
