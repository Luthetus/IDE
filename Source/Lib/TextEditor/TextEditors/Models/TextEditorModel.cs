...
public sealed class TextEditorModel
{
        ...
        int partitionSize = 4_096)
    {
        if (partitionSize < MINIMUM_PARTITION_SIZE)
            throw new ClairTextEditorException($"{nameof(PersistentState.PartitionSize)} must be >= {MINIMUM_PARTITION_SIZE}");
    ...
    public void UndoEditWithCursor(TextEditorViewModel viewModel)
    {
        if (PersistentState.EditBlockIndex <= 0)
            throw new ClairTextEditorException("No edits are available to perform 'undo' on");
    ...
    public void RedoEditWithCursor(TextEditorViewModel viewModel, bool recursive = false)
    {
        ...
        if (PersistentState.EditBlockIndex >= PersistentState.EditBlockList.Count - 1)
            throw new ClairTextEditorException("No edits are available to perform 'redo' on");
        ...
        TextEditorViewModel viewModel)
    {
        ...
        if (initialCursorPositionIndex < 0)
            throw new ClairTextEditorException($"'{nameof(initialCursorPositionIndex)}:{initialCursorPositionIndex}' < 0");
        // NOTE: DocumentLength is a valid position for the cursor to be at.
        if (initialCursorPositionIndex > CharCount)
            throw new ClairTextEditorException($"'{nameof(initialCursorPositionIndex)}:{initialCursorPositionIndex}' > {nameof(CharCount)}:{CharCount}");
        ...
        // Reposition the ViewModel InlineUiList
        {
            PersistentState.__TextEditorViewModelLiason.InsertRepositionInlineUiList(
                ...
                lineEndPositionAddedCount: lineEndPositionLazyInsertRange.localLineEndList.Count);
        }
    ...
    {
        if (columnCount < 0)
            throw new ClairTextEditorException($"{nameof(columnCount)} < 0");
        ...
        bool usePositionIndex)
    {
        ...
        if (positionIndex < 0)
            throw new ClairTextEditorException($"'{nameof(positionIndex)}:{positionIndex}' < 0");
        // NOTE: DocumentLength is a valid position for the cursor to be at.
        if (positionIndex > CharCount)
            throw new ClairTextEditorException($"'{nameof(positionIndex)}:{positionIndex}' > {nameof(CharCount)}:{CharCount}");
    ...
    public void __RemoveAt(int globalPositionIndex)
    {
        ...
        if (indexOfPartitionWithContent == -1)
            throw new ClairTextEditorException("if (indexOfPartitionWithContent == -1)");
