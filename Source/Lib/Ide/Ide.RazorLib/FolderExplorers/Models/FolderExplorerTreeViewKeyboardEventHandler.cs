...
public class FolderExplorerTreeViewKeyboardEventHandler : TreeViewKeyboardEventHandler
{
    ...
    private Task PasteClipboard(TreeViewCommandArgs commandArgs)
    {
        ...
        else
        {
            ...
            var parentDirectoryAbsolutePath = new AbsolutePath(
                parentDirectory,
                true,
                _ideService.CommonService.FileSystemProvider,
    ...
    private async Task ReloadTreeViewModel(TreeViewNoType? treeViewModel)
    {
        ...
        if (!_ideService.CommonService.TryGetTreeViewContainer(FolderExplorerState.TreeViewContentStateKey, out var treeViewContainer))
            return;
