...
public partial class IdeService
{
        ...
        return new MenuOptionValue(
            ...
            menuOptionOnClickArgs => 
            {
                MenuContainer.OpenWidget(
                    ...
                    onAfterSubmitFuncOther: new Func<string, FileTemplate?, List<FileTemplate>, Task>(
    ...
    private void Enqueue_PerformNewFile(
        string fileName,
        FileTemplate? exactMatchFileTemplate,
        List<FileTemplate> relatedMatchFileTemplatesList,
        AbsolutePath absolutePath,
        ...
        Func<Task> onAfterCompletion)
    {
        ...
        {
            var allTemplates = new[] { exactMatchFileTemplate.Value }
                .Union(relatedMatchFileTemplatesList)
                .ToArray();

            foreach (var fileTemplate in allTemplates)
            {
                var templateResult = CommonFacts.CreateFileTemplate(new FileTemplateParameter(
                    fileName,
                    absolutePath,
                    namespaceString,
                    CommonService.FileSystemProvider,
                    fileTemplate.FileExtensionNoPeriod));
    ...
    private async ValueTask Do_PerformNewDirectory(string directoryName, AbsolutePath parentDirectory, Func<Task> onAfterCompletion)
    {
        ...
        var directoryAbsolutePath = new AbsolutePath(directoryAbsolutePathString, true, CommonService.FileSystemProvider, tokenBuilder: new StringBuilder(), formattedBuilder: new StringBuilder(), AbsolutePathNameKind.NameWithExtension);
    ...
    private AbsolutePath PerformRename(AbsolutePath sourceAbsolutePath, string nextName, CommonService commonService, Func<Task> onAfterCompletion)
    {
        ...
        {
            var temporaryNextName = CommonService.FileSystemProvider.GetRandomFileName();
        ...
        return new AbsolutePath(destinationAbsolutePathString, sourceAbsolutePath.IsDirectory, CommonService.FileSystemProvider, tokenBuilder: new StringBuilder(), formattedBuilder: new StringBuilder(), AbsolutePathNameKind.NameWithExtension);
    }
