...
public sealed class TextEditorViewModelPersistentState : IDisposable, ITab, IPanelTab, IDialog, IDrag
{
    ...
    public async Task OnDragStartAsync()
    {
        ...
        var measuredHtmlElementDimensions = await CommonService.JsRuntimeCommonApi
            .MeasureElementById(
                $"ci_te_group_{TextEditorService.Group_GetTextEditorGroupState().GroupList.Single().GroupKey.Guid}")
            .ConfigureAwait(false);
    ...
    private async Task AddPanelDropzonesAsync(List<IDropzone> dropzoneList)
    {
        ...
        {
            (CommonFacts.LeftPanelGroupKey, "ci_ide_panel_left_tabs"),
            (CommonFacts.RightPanelGroupKey, "ci_ide_panel_right_tabs"),
            (CommonFacts.BottomPanelGroupKey, "ci_ide_panel_bottom_tabs"),
        };
