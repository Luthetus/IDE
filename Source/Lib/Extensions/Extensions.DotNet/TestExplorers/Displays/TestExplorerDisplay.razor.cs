...
namespace Clair.Extensions.DotNet.TestExplorers.Displays;

public sealed partial class TestExplorerDisplay : ComponentBase, IDisposable
{
    [Inject]
        ...
        if (model is null)
        {
            DotNetService.IdeService.TextEditorService.WorkerArbitrary.PostUnique(async editContext =>
            {
                ...
                DotNetService.IdeService.TextEditorService.ViewModel_Register(
                    editContext,
                    TextEditorService.__ReserveViewModelKey(TestExplorerDetailsDisplay.DetailsTextEditorViewModelKey),
                    ResourceUriFacts.TestExplorerDetailsTextEditorResourceUri,
