...
using Clair.CompilerServices.CSharpProject.CompilerServiceCase;
using Clair.CompilerServices.Css;
using Clair.CompilerServices.DotNetSolution;
using Clair.CompilerServices.Json;
using Clair.CompilerServices.Razor;
using Clair.CompilerServices.Xml;
using Clair.CompilerServices.JavaScript;
...
using Clair.Ide.RazorLib.FolderExplorers.Displays;
...
public static class InitializationHelper
{
        ...
        CancellationTokenSource _workerCancellationTokenSource)
    {
        ...
        if (DotNetService.CommonService.ClairHostingInformation.ClairHostingKind == ClairHostingKind.Photino)
        {
            await DotNetService.CommonService.JsRuntimeCommonApi.JsRuntime.InvokeVoidAsync("clairIde.preventDefaultBrowserKeybindings").ConfigureAwait(false);
        }
        ...
        if (DotNetService.CommonService.ClairHostingInformation.ClairPurposeKind == ClairPurposeKind.Ide)
        {
    ...
    public static void CleanProjectOnClick(DotNetService DotNetService, string projectAbsolutePathString)
    {
        ...
        var solutionAbsolutePath = new AbsolutePath(projectAbsolutePathString, false, DotNetService.CommonService.FileSystemProvider, tokenBuilder: new StringBuilder(), formattedBuilder: new StringBuilder(), AbsolutePathNameKind.NameWithExtension);
    ...
    public static void BuildSolutionOnClick(DotNetService DotNetService, string solutionAbsolutePathString)
    {
        ...
        var solutionAbsolutePath = new AbsolutePath(solutionAbsolutePathString, false, DotNetService.CommonService.FileSystemProvider, tokenBuilder: new StringBuilder(), formattedBuilder: new StringBuilder(), AbsolutePathNameKind.NameWithExtension);
    ...
    public static async Task SetSolution(DotNetService DotNetService, DotNetAppData dotNetAppData)
    {
        ...
        var slnAbsolutePath = new AbsolutePath(
            solutionMostRecent,
            false,
